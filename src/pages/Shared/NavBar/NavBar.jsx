    ...
    ${isSticky ? 'bg-[#FF9466]' : 'bg-transparent'}`}>
        ...
        <div className="navbar-start">
          <div className="dropdown">
            ...
            <ul tabIndex={0} className={`menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 ${textColorClass}`}>
              {navOptions}
            </ul>
          </div>
          {/* 'text-[#FF9466]' */}
          <a className={`font-bold text-xl md:px-12 ${textColorClass}`}>Travel-<span className={`${isSticky ? 'text-[#FFFFFF]' : 'text-[#FF9466]'}`} >Swift</span></a>
        </div>
        <div className="navbar-center hidden lg:flex ">
          <ul className={`menu menu-horizontal px-1 font-semibold text-base ${textColorClass}`}>
            {navOptions}
          </ul>
        </div>
      </div>
